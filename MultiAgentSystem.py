...
#     llm = ChatOpenAI(model=MODEL_NAME, temperature=0.2, max_tokens=1000)
...
#     """
#     # def __init__(self, tenant_id: str, bot_id: str,session_id: str):
...
#     #     self.bot_instructions = []  # ✅ Generic list, no assumptions

#     #     if not all([self.qdrant_client, self.openai_client, self.llm]):
#     #         raise ConnectionError("One or more required clients (Qdrant, OpenAI) failed to initialize. Please check API keys and connection details.")


#     #     self._load_config_from_db()
#     #     self._load_chat_history()

#     #     # 2. NOW instantiate tools based on enabled_tools
#     #     self._initialize_tools()

#     #     # Initialize and compile the LangGraph workflow
#     #     self.graph_executor = self._initialize_graph()

#     #     logger.info(f"Initialized MultiAgentSystem with LangGraph for tenant '{tenant_id}', bot '{bot_id}'")
...
#         # HubSpot
#         if "hubspot" in self.enabled_tools and HUBSPOT_ACCESS_TOKEN:
...
#     #     """Loads bot-specific configuration like KB collections and bot personality."""
...
#     #         if not custom_bot:
#     #             logger.warning(f"No custom bot found for bot_id={self.bot_id}. Using defaults.")
#     #             self.bot_details = {
#     #                 "name": "Assistant",
#     #                 "tone": "professional",
#     #                 "industry": "general",
...
#     #         self.kb_summaries = []
#     #         self.kb_catalog = []   # ✅ CRITICAL INITIALIZATION

#     #         kb_ids = custom_bot.kb_ids if isinstance(custom_bot.kb_ids, list) else []
...
#     #                 # ✅ Structured catalog for KB router & KB agent
...
#     #             logger.info(
#     #                 f"Loaded {len(self.kb_catalog)} KB collections "
...
#     #     except Exception as e:
#     #         logger.error(f"Error loading bot config from DB: {e}", exc_info=True)
#     #     finally:
#     #         session.close()
...
#                 tenant_id=self.tenant_id, bot_id=self.bot_id,session_id=self.session_id
...
#             session.commit()
#             self.chat_memory.append({"query": query, "response": response})
#         except Exception as e:
#             session.rollback()
...
#                 formatted_history += f"User: {turn.get('query', '')}\nAI: {turn.get('response', '')}\n"
#             formatted_history += "\n"
...
#             - tools → actionable tasks requiring external tool execution (send, book, find, calculate, lookup, schedule, generate, create, fetch, update, etc.)
...
#             -------------------------
#             Tool Catalog (Use ONLY if necessary):
#             {tool_metadata_section}

#             -------------------------
#             Routing Rules:
#             1. If the query is a greeting (hello / hi / hey / good morning) → respond: greeting
#             2. If the user is asking for INFORMATION that likely exists in the KB → respond: kb
#             3. If the user is ASKING TO PERFORM an ACTION and a suitable tool exists → respond: tools
#             4. If unsure between kb and tools:
#             - If it's informational → kb
#             - If it's operational, requiring execution → tools
#             5. If NO KB exists and tools exist → default to tools
#             6. If NO tools exist and KB exists → default to kb
...
#                 next_agent = "kb" if kb_summary_text else "tools"
...
#             # Safe fallback
#             return {"next_agent": "kb" if kb_summary_text else "tools"}
...
#         purpose = self.bot_details.get('purpose', 'assist you')
#         # Take only first sentence to keep greeting short
#         short_purpose = purpose.split('.')[0].strip() if purpose else 'assist you'
#         output = f"Hi! I'm {name}, {short_purpose}. How can I help you today?"
...
#             logger.info("No KB configured. Falling back to general LLM answer.")
...
#             logger.info(f"Searching KB {kb['kb_id']} ({kb['collection_name']})")
...
#             except Exception as e:
#                 logger.error(f"KB search failed for KB {kb['kb_id']}: {e}")
        
#         # ---- DEDUPLICATE CONTEXT CHUNKS (ORDER PRESERVED) ----
#         all_context_chunks = list(dict.fromkeys(all_context_chunks))
#         # ---- FALLBACK USING KB SUMMARIES ----
#         if not all_context_chunks:
#             fallback_summaries = []

#             for kb in self.kb_catalog:
#                 if kb["kb_id"] in kb_ids_to_search and kb.get("summary"):
#                     fallback_summaries.append(kb["summary"])
...
#         Below are high-level descriptions of the selected knowledge bases:

#         \"\"\"{combined_summary}\"\"\"

#         If the answer can be inferred from these descriptions, answer concisely.
#         If not, say you don't have enough information.
#         """
#                 resp = self.llm.invoke([HumanMessage(content=prompt)]).content.strip()
#                 return {"agent_output": resp}

#             return {"agent_output": "I couldn't find any relevant information in the knowledge base."}
...
#         - If partial information exists, provide the best possible answer from it
...
#         - If exact answer is not present → say: "Based on available information..." and respond
...
#         logger.info(f"KB Agent: Answer length={len(response)} chars")

#         return {"agent_output": response}

#     # def tools_agent(self, state: AgentState, verbose: bool = True) -> Dict[str, str]:
...
#     # You are {self.bot_details.get('name', 'a helpful assistant')}.
#     # Your tone should be {self.bot_details.get('tone', 'professional')}.
...
#     # 2. Ensure all required inputs are present
#     # 3. Ask the user if information is missing
#     # 4. Follow Thought → Action → Action Input format
    
#     # CRITICAL:
#     #     - After you receive a tool Observation with useful results,
#     #     you MUST respond with:
#     #     Final Answer: <natural language response>
#     #     - Do NOT write "Thought:" after the final tool call.
#     #     - Do NOT call any more tools unless strictly necessary.

#     # Begin!
#     # """

#     #         result = agent_executor.invoke({"input": prompt_with_history})
#     #         final_response = result.get("output", "I'm sorry, I couldn't process that request.")
#     #         return {"agent_output": final_response}

#     #     except Exception as e:
#     #         logger.exception(f"Tools Agent error: {e}")
#     #         return {"agent_output": "Sorry, an internal error occurred."}
...
#                 agent=AgentType.ZERO_SHOT_REACT_DESCRIPTION,
#                 verbose=verbose,
#                 handle_parsing_errors=False,  # FAIL FAST (important)
#                 max_iterations=3,
#             )
...
#             - DO NOT always start with "Based on the information available" — vary your opening
#             - DO NOT always end with "Is there anything else..." — only ask a follow-up if it genuinely makes sense
#             - DO NOT add a word limit — respond based on what the answer requires (maximum limit is 50 words)
...
#             contact_properties['firstname'] = "Shubham"
#             contact_properties['lastname'] = "Shahane"
...
#             return mas.nearby_facilities_tool.process(location=location, facility_type=facility_type)
#         except json.JSONDecodeError:
...
#                 return "Error: Missing 'origin' or 'destination' in JSON input."
#             result = mas.commute_time_tool.process(
#                 origin=origin,
#                 destination=destination,
#                 future_date_str=future_date
#             )
#             return result["message"] if isinstance(result, result.get("message")) else str(result)
#         except json.JSONDecodeError:
...
Provide only the rephrased query.
    ...
    def tools_agent(self, state: AgentState, verbose: bool = False) -> Dict[str, str]:
        ...
        if self.mode == "kb_strict" and not _build_tool_map(self, exclude_kb=True):
            logger.info("[Tools Agent] kb_strict with no tools available")
            return {"agent_output": "No external tools are enabled for this bot."}
