...
@register_node("whatsappDownloadMediaNode")
class WhatsappDownloadMediaNode(BaseNode):
    """Download WhatsApp media by media_id."""

    def execute(self, inputs):
        form_data = resolve_form_data(self.form_data or {}, inputs)
        tenant_id = get_tenant_id(inputs, form_data)
        if not tenant_id:
            raise ValueError("whatsappDownloadMediaNode: Missing tenant_id")

        media_id = form_data.get("media_id") or infer_media_id(inputs)
        if not media_id:
            raise ValueError("whatsappDownloadMediaNode: Missing media_id")

        strategy = WhatsAppExportStrategy()
        result = strategy.download_media(
            str(tenant_id),
            {
                **form_data,
        ...
        return {
            "status": "success",
            "node": "whatsappDownloadMediaNode",
            "result": result,
            "media_id": str(media_id),
        }
