...
class BankCharge(db.Model):
    __tablename__ = 'tbl_bank_charges'

    bank_charge_id = db.Column(
        db.Integer,
        primary_key=True,
        autoincrement=True
    )
    ...
    applies_to = db.Column(db.String(50), nullable=True)

    condition_key = db.Column(db.String(50), nullable=True)
    condition_value = db.Column(db.String(50), nullable=True)
    ...
    def __repr__(self):
        return f"<BankCharge {self.carrier} | {self.charge_name}>"
