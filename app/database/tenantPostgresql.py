...
class Tenant(Base):
    ...
    tenant_GSTNo = Column(String(100), nullable=True)
...
DATABASE_URL = os.getenv(
    ...
    'postgresql+psycopg2://postgres:123@localhost:5432/db_botbuilder'
)
...
def update_record(model, record_id, **kwargs):
    try:
        record = session.query(model).filter_by(login_id=record_id).first()  # Updated to use the `login_id` field
        if not record:
