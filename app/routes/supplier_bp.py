                ...
                "line_items": [
                    {
                        ...
                        "lead_time": item.lead_time,
                        "hsn_code": item.hsn_code,
                        # Backward-compatible: some deployments/models do not define moq/soq.
                        "moq": getattr(item, "moq", getattr(item, "minimum_order_quantity", None)),
                        "soq": getattr(item, "soq", getattr(item, "standard_order_quantity", None)),
                        "weight_of_package": item.weight_of_package,
                        "dim_of_package": item.dim_of_package,
                        "is_selected": item.is_selected,
            ...
            with session.begin_nested():
                ...
                try:
                    ...
                    offered_qty = parse_quantity(
                        get_first_present(material, "offered_quantity","required_quantity","quantity")
                    )
                    moq = material.get("moq")
                    soq = material.get("soq")
                    ...
                    if supplier_line:
                        ...
                        supplier_line.moq = moq          # <-- add
                        supplier_line.soq = soq
